...
- Running SQL against real PostgreSQL or MySQL servers. `ci.yml:72-95` and
  `:97-120` already do that — 10 and 9 tests against `postgres:16` and `mysql:8`
  service containers — and neither is what a fresh consumer hits first. Step 3b
  covers the one thing CI does not: extras resolution from a built artefact.
  Widen this only if step 3b or the SQLite runs surface something.
...
- Everything is pushed to `latingate/eventlog-pro`. Verified on 2026-08-14:
  anonymous `git ls-remote` returns `refs/heads/main` at `f155821`, matching
  local `HEAD`. `f155821` adds only plan and rule files on top of `56d190d`;
...
1. The commit SHA `f155821` — reproducible, and exactly what plan 002 will tag.
...
2. **Install from git, with the Django extra.** PEP 508 direct-reference form —
   ...
   ```powershell
   pip install --no-cache-dir "eventlog-pro[django] @ git+https://github.com/latingate/eventlog-pro@f155821"
   ```
...
3b. **Check the `[postgres]` and `[mysql]` extras resolve.** Nothing else tests
   ...
   server.
   ```powershell
   pip install --no-cache-dir "eventlog-pro[postgres,mysql] @ git+https://github.com/latingate/eventlog-pro@f155821"
...
4b. **Strict pure-Python check, in a venv that has nothing else in it.** Step 4
   ...
   & "$pure\.venv\Scripts\python.exe" -m pip install --no-cache-dir "eventlog-pro @ git+https://github.com/latingate/eventlog-pro@f155821"
...
**Executed 2026-08-14 against `f155821`, Python 3.12.10, Windows. All checks
passed.** The project is at `~\PycharmProjects\eventlog-consumer-smoke`, left in
place for the browser check in step 6.
